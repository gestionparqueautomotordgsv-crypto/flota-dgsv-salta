...
def panel(df, mostrar_azul=True):
    df_f=df.copy()
    ...
    if alerta>0:
        st.markdown(f"<div style='background:#fff3cd;border:1px solid #ffe69c;padding:12px;border-radius:8px;margin-top:15px;color:#664d03;font-size:16px'>🟡 <b>ALERTA KM: {alerta} próximos al servi - DENTRO DE NORMAL</b></div>", unsafe_allow_html=True)
        ...
        if mostrar_azul:
            st.markdown(f"<div style='margin-top:10px;padding:8px;background:#e2e3e5;border-radius:6px'>🔵 <b>ALERTA PARA SERVI: {alerta} móviles con alerta de km</b></div>", unsafe_allow_html=True)
...
with t1:
    panel(df_2r, mostrar_azul=True)
with t2:
    panel(df_4r, mostrar_azul=False)
